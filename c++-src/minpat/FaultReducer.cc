...
FaultReducer::FaultReducer(
  ...
  const vector<FaultInfo>& fault_info_list,
  const JsonValue& option
) : mNetwork{network},
    mFaultInfoList{fault_info_list}
{
  if ( option.is_object() ) {
...
{
  // ノードごとの故障リストを作る．
  // 同時に fault_list, tv_list, max_fault_id を求める．
  SizeType n = mFaultInfoList.size();
