...
)
{
  if ( has_x ) {
    ...
    if ( has_previous_state ) {
      return nsFsimNaiveBside3::new_Fsim(fault_list);
    }
    else {
      return nsFsimNaiveCombi3::new_Fsim(fault_list);
    }
  }
  else {
    // 2値バージョン
    if ( has_previous_state ) {
      return nsFsimNaiveBside2::new_Fsim(fault_list);
    }
    else {
      return nsFsimNaiveCombi2::new_Fsim(fault_list);
    }
  }
}
