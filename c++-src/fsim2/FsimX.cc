...
{
  return static_cast<std::unique_ptr<Fsim2Impl>>(new FSIM2_CLASSNAME{network});
}
...
// @brief コンストラクタ
FSIM2_CLASSNAME::FSIM2_CLASSNAME(
  const TpgNetwork& network
)
{
  set_network(network);
...
)
{
  ...
  for ( auto tpgnode: network.node_list() ) {
    ...
    if ( tpgnode->is_ppi() ) {
      ...
      node = make_output(inode, tpgnode->output_id());
  ...
  mEventQ.init(max_level, mNodeArray.size(), no);
}

// @brief FFR のリストを返す．
const vector<SimFFR>&
FSIM2_CLASSNAME::_ffr_list() const
{
  return mFFRArray;
}
...
// @brief 対象の故障をセットする．
void
FSIM2_CLASSNAME::set_fault_list(
...
void
FSIM2_CLASSNAME::clear_skip_all()
{
  for ( auto& f: mFaultList ) {
    f->set_skip(false);
  }
...
FSIM2_CLASSNAME::spsfp_diffvector()
{
  SizeType npo = ppo_num();
  DiffVector ans(npo, false);
  for ( SizeType i = 0; i < npo; ++ i ) {
    ans[i] = mEventQ.prop_val(i);
  }
...
void
FSIM2_CLASSNAME::_do_simulation(
...
)
{
  ...
  PackedVal mask = 1ULL;
  for ( auto bitpos = 0; bitpos < ffr_num; ++ bitpos, mask <<= 1 ) {
    if ( obs & mask ) {
      for ( auto ff: ffr_buff[bitpos]->fault_list() ) {
	if ( ff->skip() || ff->obs_mask() == PV_ALL0 ) {
	  continue;
	}
	auto tpg_f = ff->tpg_fault();
	mDetFaultArray.push_back(tpg_f);
	DiffVector dv(ppo_num(), false);
	for ( SizeType i = 0; i < ppo_num(); ++ i ) {
	  auto dbits = mEventQ.prop_val(i);
	  if ( dbits & mask ) {
	    dv[i] = true;
	  }
	}
...
PackedVal
FSIM2_CLASSNAME::_foreach_faults(
...
#else
    #error "Neither FSIM2_VAL2 nor FSIM2_VAL3 are defined"
#endif
    cond &= val;
  }
  return cond;
