...
TEST_F(TpgNetworkTest, empty)
{
  auto network = mBuilder.wrap_up();

  ASSERT_TRUE( network.is_valid() );
  EXPECT_EQ( 0, network.node_num() );
  EXPECT_THROW( network.node(0),
		std::out_of_range );
  auto node_list = network.node_list();
  EXPECT_EQ( 0, node_list.size() );

}

TEST_F(TpgNetworkTest, xor2)
{
