...
ConflictChecker::check_mandatory_condition()
{
  ...
  engine.add_cur_node_list(node_list);
  engine.add_prev_node_list(node_list);
  for ( SizeType i1 = 0; i1 < mFaultNum - 1; ++ i1 ) {
    auto fault1 = mFaultList[i1];
...
ConflictChecker::check_final()
{
  ...
  for ( SizeType i1 = 0; i1 < nffr - 1; ++ i1 ) {
	...
	continue;
      ...
      StructEngine engine(network());
      engine.add_subenc(std::unique_ptr<SubEnc>{bd_enc1});
      engine.add_subenc(std::unique_ptr<SubEnc>{bd_enc2});
      engine.add_prev_node(ffr1->root());
      engine.add_prev_node(ffr2->root());
