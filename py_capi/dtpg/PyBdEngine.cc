...
// Python 用のタイプ定義
PyTypeObject BdEngine_Type = {
  PyVarObject_HEAD_INIT(nullptr, 0)
  // 残りは PyBdEngine::init() 中で初期化する．
};
...
{
  auto obj = reinterpret_cast<BdEngine_Object*>(self);
  obj->mVal.~BdEngine();
...
)
{
  auto& val = PyBdEngine::_get_ref(self);
  try {
    return PySatLiteral::ToPyObject(val.prop_var());
  }
  catch ( std::exception err ) {
    std::ostringstream buf;
    buf << "exception" << ": " << err.what();
...
// @brief BdEngine オブジェクトを使用可能にする．
bool
PyBdEngine::init(
  PyObject* m
)
{
  BdEngine_Type.tp_name = "BdEngine";
  BdEngine_Type.tp_basicsize = sizeof(BdEngine_Object);
  BdEngine_Type.tp_itemsize = 0;
  BdEngine_Type.tp_dealloc = dealloc_func;
  BdEngine_Type.tp_flags = Py_TPFLAGS_DEFAULT;
  BdEngine_Type.tp_doc = PyDoc_STR("Python extended object for BdEngine");
  BdEngine_Type.tp_methods = methods;
  BdEngine_Type.tp_getset = getsets;
  BdEngine_Type.tp_new = new_func;
  if ( !PyModule::reg_type(m, "BdEngine", &BdEngine_Type) ) {
    goto error;
  }
...
{
  return Py_IS_TYPE(obj, &BdEngine_Type);
}

// @brief PyObject から BdEngine を取り出す．
BdEngine&
PyBdEngine::_get_ref(
  PyObject* obj
)
{
  auto my_obj = reinterpret_cast<BdEngine_Object*>(obj);
  return my_obj->mVal;
}

// @brief BdEngine を表すオブジェクトの型定義を返す．
PyTypeObject*
PyBdEngine::_typeobject()
{
  return &BdEngine_Type;
}
